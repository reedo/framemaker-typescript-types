...
interface $ {
   ...
   * Assigning error text to this property generates a run-time error; however, the preferred way to generate a run-time
   ...
   *   0x0040 ( 64)  Enables excessive garbage collection. Usually, garbage collection starts when the number of objects
   *                 has increased by a certain amount since the last garbage collection. This flag causes ExtendScript
   *                 to garbage collect after almost every statement. This impairs performance severely, but is useful
   *                 when you suspect that an object gets released too soon.
  ...
  /**
   * A high-resolution timer, measuring the time in microseconds. The timer starts when ExtendScript is initialized
   * during the application startup sequence. Every read access resets the timer to Zero.
   */
   ...
   * The string contains five characters in the form LL_RR, where LL is an ISO 639 language specifier, and RR is an
   * ISO 3166 region specifier.Initially, this is the value that the application or the platform returns for the current
   * user. You can set it to temporarily change the locale for testing. To return to the application or platform
   ...
   * Each object has the properties left, top, right, bottom, which contain the four corners of each screen in global
   * coordinates.A property primary is true if that object describes the primary display.
   */
   ...
   * When true, any attempt to write to a read-only property causes a runtime error. Some objects do not permit the
   ...
   * 
   * @param condition - A string containing a JavaScript statement to be used as a condition. If the statement evaluates
   *                    to true or nonzero when this point is reached, execution stops.
   */
   ...
   * During a sleep period, checks at 100 millisecond intervals to see whether the sleep should be terminated. This can
   * happen if there is a break request, or if the script timeout has expired.
   * 
...
interface Object {
   ...
   * This function serializes the object, so that it can, for example, be passed between engines. Pass the returned
   * string back to eval() to recreate the object. Works only with built-in classes.
   */
  toSource(): string;

  /**
   * Creates and returns a string representing this object.
   * Many objects (such as Date) override this method in favor of their own implementation. If an object has no string
   * value and no user-defined toString() method, the default method returns [object type], where type is the object
   * type or the name of the constructor function that created the object.
   */
  toString(): string;
   ...
   * If the object has no primitive value, returns the object itself.Note that you rarely need to call this method
   * yourself.The JavaScript interpreter automatically invokes it when encountering an object where a primitive value is
   * expected.
   */
  valueOf(): Object;
   ...
   * This function can accept, modify, or reject a new value that the user, application, or a script has attempted to
   * place in a property.
   * 
   * @param name - The name of the property to watch.
   * @param func - The function to be called when the value of this property changes. This function must three
   *               arguments, and return as its result the value to be stored in the property. The arguments are:
...
interface ArrayConstructor {
   ...
   * Takes any number of parameters, which become the elements of the array, or a single value which becomes the length
   * of an empty array. Note that you cannot create a one-element array, as the single parameter value is interpreted as
   * the length. Returns the new array.
   * 
   * @param arrayLength - If no other parameters are passed, the initial length of the empty array.
...
interface Array<T> {
   ...
   * The original array is unchanged.If an array is provided as a parameter to concat(), each of its elements are
   * appended as separate array elements at the end of the new array.Returns a new array, the result of concatenation
   * the given values to the end of the original array.
   * 
   ...
   * 
   * @param delimiter - A string used to separate each element of the array. If omitted, the array elements are
   ...
   * The slice begins with the index start, and continues up to, but not including the index, end.If start or end is a
   * negative number, the indexed is resolved counting backwards from the end of the array resulting in the element
   * array[array. length + negativeIndex]. Returns a new array containing elements array[start] through array[end-1].
   */
   ...
   * 
   * @param userFunction - A user-supplied function of the form userFunction(a, b) which returns less than 0 if a is
   *                       greater than b, 0 if a and b are equal, and greater than 0 if b is greater than a.
   */
  sort(userFunction?: (a: T, b: T) => number): this;

  /**
   * Removes num elements from the array beginning with index, start.
   * Optionally insert new elements beginning at index start.To ensure contiguity, elements are moved up to fill in any
   * gaps.Returns a new array containing any elements deleted from the original array.
   * 
   * @param start - The index of the first element to remove. Negative values are relative to the end of the array.
   * @param deleteCount - The number of array elements to remove, including start. If omitted, all elements from array
   *                      index start to the end of the array are removed.
   * @param values - A list of one or more values to be added to the array starting at index start. Must specify a value
   *                 for num, to use this argument.
   */
  ...
  /**
   * Creates a string representation of this object that can be fed back to eval() to re-create an object. Works only
   ...
   * Yields the same result as array. join() when called without a parameter.Returns a comma-separated list of all the
...
interface DateConstructor {
   ...
   * @param hours - An integer value from 0 (midnight) to 23 (11 PM). If this argument is not supplied, its value is set
   *                to 0.
   ...
   * 
   * @param year - The year expressed in four digits, for example, 2001. To indicate for a year from 1900 to 1999, you
   *               can specify a value from 0 to 99.
   * @param month - An integer value from 0 (Jan) to 11 (Dec).
   * @param day - An integer value from 1 to 31, If this argument is not supplied, its value is set to 0.
   * @param hours - An integer value from 0 (midnight) to 23 (11 PM). If this argument is not supplied, its value is set
   *                to 0.
...
interface Date {
   ...
   * This method can also set month and date if those arguments are specified. Returns the number of milliseconds
   * between the new date and midnight, January 1, 1970.
   * 
  ...
  /**
   * Sets the year of a specified Date object according to local time, as a difference between the current year and
   * 1900.
   * Returns the number of milliseconds between the date set and midnight, January 1, 1970.
   * 
   * @param year - An integer value indicating the year to set. The method interprets a 1- or 2- digit value to mean the
   *               1900s; for example, 13 is interpreted to mean 1913.
   */
  ...
  /**
   * Returns a string value representing the date and time stored in the Date object in human readable format.
   * Returns the following string is an example of the format returned by this method: Mon Aug 13, 10:54:21 GMT-0700
   * 2001.
   */
  toString(): string;
...
interface FunctionConstructor {
   ...
   * 
   * @param arguments - The list of formal arguments, separated by commas. The formal arguments can also be supplied one
   *                    by one; in this case, the last argument to the Function constructor is considered to be the
   *                    function body.
...
interface String {
   ...
   * Those values are concatenated with the original string, the result is returned. The original string is not
   * effected.Returns the concatenated string.
   * 
  ...
  /**
   * Returns the index within the string of the first occurrence of the specified string, starting the search at
   * fromIndex if provided.
   * 
   ...
   * The string is searched backward, starting at fromIndex.Returns the index within the string where the last
   * occurrence of searchValue was found, or -1 if it was not found.
   * 
   * @param searchValue - The string for which to search.
   * @param offset - The starting offset of the search.
   */
   ...
   * UNDOCUMENTED
   * 
   * @param search - UNDOCUMENTED
   */
  search(search: RegExp): number;

  /**
   * Extracts a substring of the given string and returns it as a new string.
   * The substring begins at startSlice, and includes all characters up to, but not including the character at the index
   * endSlice. A negative value indexes from the end of the string.For example, a negative value for startSlice is
   * resolved as: string. length + startSlice.The original string is unchanged.Returns a substring of characters from
   * the given string, starting at startSlice and ending with endSlice-1.
   * 
   ...
   * The substrings are created by breaking the original string at places that match delimiter, the delimiter characters
   * are removed.Returns an array whose elements are the substrings.
   * 
   * @param delimiter - Specifies the string to use for delimiting. If delimiter is omitted, the array returned contains
  ...
  /**
   * Returns a string containing the characters beginning at the specified index, start, through the specified number of
   * characters.
   ...
   * The original string is unchanged.Returns a substring of characters from the given string, starting at indexA and
...
interface RegExpConstructor {
   ...
   * @param flags - Flags that control how the conversion is performed. A string containing any combination of the
   *                letters i, m, g: "i" -- ignore case in pattern matching "m" -- treat the string as multiple lines
...
interface RegExp {
   ...
   * The return value is an array of matches, with the first element containing the match, and successive elements
   * containing the results of any matching subexpression in their order of appearance. If there is no match, the result
   * is null.
   * 
...
interface FileConstructor {
   ...
   * 
   * @param path - The full or partial path name of the file,in platform-specific or URI format. The value stored in the
   *               object is the absolute path. The file that the path refers to does not need to exist.If the path
   *               refers to an existing folder: The File function returns a Folder object instead of a File object.
   ...
   * This is a class property accessed through the File constructor. Valid values are "Windows", "Macintosh", and
   * "Unix".
   */
  readonly fs: string;
   ...
   * All special characters are encoded in UTF-8 and stored as escaped characters starting with the percent sign
   * followed by two hexadecimal digits. For example, the string "my file" is encoded as "my%20file".
   ...
   * 
   * @param name - The encoding name. Typical values are "ASCII", "binary", or "UTF-8".For a complete list of supported
   * encodings, see the JavaScript Tools Guide.
   */
   ...
   * files, and creates new File objects to represent the selected files.
   * If the user clicks OK, returns a File object for the selected file, or an array of objects if multiple files are
   * selected.
   * If the user cancels, returns null.
   * 
   * @param prompt - The prompt text, displayed if the dialog allows a prompt.
   * @param filter - A filter that limits the types of files displayed in the dialog. In Windows,a filter expression
   *                 such as "Javascript files:*.jsx;All files:*.*". In Mac OS, a filter function that takes a File
   *                 instance and returns true if the file should be included in the display, false if it should not.
   ...
   * Opens the built-in platform-specific file-browsing dialog in which a user can select an existing file location to
   * which to save information, and creates a new File object to represent the selected file location.
   * If the user clicks OK, returns a File object for the selected file location.
   * If the user cancels, returns null.
   * 
   * @param prompt - The prompt text, displayed if the dialog allows a prompt.
   * @param filter - In Windows only, a filter that limits the types of files displayed in the dialog. In Windows only,
   *                 a filter expression such as "Javascript files:*.jsx;All files:*.*". Not used In Mac OS.
   */
...
interface File {
   ...
   * One of the encoding constants listed in the JavaScript Tools Guide. If the value is not recognized, uses the system
   * default encoding.A special encoder, BINARY, is used to read binary files. It stores each byte of the file as one
   * Unicode character regardless of any encoding. When writing, the lower byte of each Unicode character is treated as
   ...
   * Typically set by the file system, but a script can set it. Setting this value clears any error message and resets
   * the error bit for opened files. Contains the empty string if there is no error.
   */
  error: string;
   ...
   * If the object references a file-system alias or shortcut, the flag is altered on the alias, not on the original
   * file.
   */
  hidden: boolean;
   ...
   * Can be set only for a file that is not open, in which case it truncates or pads the file with 0-bytes to the new
   * length.
   */
  length: number;
   ...
   * If the referenced file is a file-system alias or shortcut, the flag is altered on the alias, not on the original
   * file.
   */
  readonly: boolean;
   ...
   * 
   * @param target - A string with the URI path to the target location, or a File object that references the target
   *                 location.
   */
   ...
   * You can use this method to run scripts, launch applications, and so on.Returns true immediately if the application
   ...
   * If no base path is supplied, the URI is relative to the path of the current folder.Returns a string containing the
   * relative URI.
   * 
   ...
   * Returns true if the file was opened successfully.The method attempts to detect the encoding of the open file. It
   * reads a few bytes at the current location and tries to detect the Byte Order Mark character 0xFFFE. If found, the
   * current position is advanced behind the detected character and the encoding property is set to one of the strings
   * UCS-2BE, UCS-2LE, UCS4-BE, UCS-4LE, or UTF-8. If the marker character is not found, it checks for zero bytes at the
   * current location and makes an assumption about one of the above formats (except UTF-8). If everything fails, the
   * encoding property is set to the system encoding.
   * IMPORTANT: Be careful about opening a file more than once. The operating system usually permits you to do so, but
   * if you start writing to the file using two different File objects, you can destroy your data.
   * 
   * @param mode - The read-write mode, a single-character string. One of these characters: r (read) Opens for reading.
   *               If the file does not exist or cannot be found, the call fails. w (write) Opens a file for writing.
   *               If the file exists, its contents are destroyed. If the file does not exist, creates a new, empty
   *               file. e (edit) Opens an existing file for reading and writing. a (append) Opens an existing file for
   *               reading and writing, and moves the current position to the end of the file.
  ...
  /**
   * Opens the built-in platform-specific file-browsing dialog, in which the user can select an existing file or files,
   * and creates new File objects to represent the selected files.
   * Differs from the class method openDialog() in that it presets the current folder to this File object’s parent
   * folder and the current file to this object’s associated file.
   ...
   * Returns a string that contains up to the specified number of characters. If a number of characters is not supplied,
   * reads from the current position to the end of the file. If the file is encoded, multiple bytes might be read to
   ...
   * Line feeds are recognized as CR, LF, CRLF or LFCR pairs.If the file is encoded, multiple bytes might be read to
   * create a single Unicode character. Returns a string that contains the character.
   */
  readch(): string;
   ...
   * Line feeds are recognized as CR, LF, CRLF or LFCR pairs.. If the file is encoded, multiple bytes might be read to
   * create single Unicode characters. Returns a string that contains the text.
   */
  readln(): string;

  /**
   * Deletes the file associated with this object from disk immediately, without moving it to the system trash.
   * Does not resolve aliases; instead, deletes the referenced alias or shortcut file itself. Returns true if the file
   ...
   * Does not resolve aliases, but renames the referenced alias or shortcut file itself. Returns true if the file was
   ...
   * If successful, creates and returns a new File object that points to the resolved file system element. Returns null
   * if this object does not refer to an alias, or if the alias could not be resolved.
   */
  resolve(): File;

  /**
   * Opens the built-in platform-specific file-browsing dialog, in which the user can select an existing file location
   * to which to save information, and creates a new File object to represent the selected file.
   * Differs from the class method saveDialog() in that it presets the current folder to this File object’s parent
   * folder and the file to this object’s associated file.
   * If the user clicks OK, returns a File object for the selected file.
   * If the user cancels, returns null.
   * 
   * @param prompt - A string containing the prompt text, if the dialog allows a prompt.
   * @param filter - In Windows only, a filter that limits the types of files displayed in the dialog. In Windows only,a
   *                 filter expression such as "Javascript files:*.jsx;All files:*.*". Not used In Mac OS.
   */
  saveDlg(prompt?: string, filter?: any): File;

  /**
   * Seeks to a given position in the file.
   * The new position cannot be less than 0 or greater than the current file size. Returns true if the position was
   * changed.
   * 
   * @param pos - The new current position in the file as an offset in bytes from the start, current position, or end,
   *              depending on the mode.
   * @param mode - The seek mode. One of: 0: Seek to absolute position, where pos=0 is the first byte of the file. This
   *               is the default. 1: Seek relative to the current position. 2. Seek backward from the end of the file.
   */
   ...
   * You can supply multiple text values; the strings are concatenated to form a single string.For encoded files,
   * writing a single Unicode character may write multiple bytes. Returns true if the write was successful.
   * IMPORTANT: Be careful not to write to a file that is open in another application or object, as this can overwrite
   * existing data.
   * 
   ...
   * You can supply multiple text values. The strings are concatenated into a single string, which is written in the
   * file followed by one line-feed sequence, of the style specified by this object's linefeed property.For encoded
   * files, writing a single Unicode character may write multiple bytes.Returns true if the write was successful.
   * IMPORTANT: Be careful not to write to a file that is open in another application or object, as this can overwrite
   * existing data.
   * 
   * @param text - A text string to be written.
...
interface FolderConstructor {
   ...
   * If the path name refers to an already existing disk file, a File object is returned instead.Returns the new Folder
   * object.
   * 
   * @param path - The absolute or relative path to the folder associated with this object, specified in URI format. The
   * value stored in the object is the absolute path.The path need not refer to an existing folder. If the path refers
   * to an existing file, rather than a folder: The Folder() function returns a File object instead of a Folder object.
   ...
   * Assign a Folder object or a string containing the new path name to set the current folder. This is a class property
  ...
  /**
   * Opens the built-in platform-specific file-browsing dialog, and creates a new File or Folder object for the selected
   * file or folder.
...
interface Folder {
  ...
  /**
   * TThe Folder object for the folder that contains this folder, or null if this object refers to the root folder of a
   * volume.
   */
  readonly parent: Folder;
   ...
   * 
   * @param mask - A search mask for file names, specified as a string or a function. A mask string can contain question
   *               mark (?) and asterisk (*) wild cards. Default is "*", which matches all file names. Can also be the
   *               name of a function that takes a File or Folder object as its argument. It is called for each file or
   *               folder found in the search; if it returns true, the object is added to the return array.
   *               NOTE: In Windows, all aliases end with the extension .lnk. ExtendScript strips this from the file
   *               name when found, in order to preserve compatibility with other operating systems. You can search for
   *               all aliases by supplying the search mask "*.lnk", but note that such code is not portable.
   */
   ...
   * Folders must be empty before they can be deleted. Does not resolve aliases; instead, deletes the referenced alias
   * or shortcut file itself. Returns true if the file was successfully removed.
   * IMPORTANT: Cannot be undone. It is recommended that you prompt the user for permission before deleting.
   */
  remove(): boolean;
   ...
   * Does not resolve aliases, but renames the referenced alias or shortcut file itself. Returns true if the folder was
   * successfully renamed.
   * 
  ...
  /**
   * Attempts to resolve the file-system alias or shortcut that this object refers to.
   * If successful, creates and returns a new Folder object that points to the resolved file system element. Returns
   * null if this object does not refer to an alias, or if the alias could not be resolved.
   */
  resolve(): Folder;
...
interface Socket {
   ...
   * Deleting the object also closes the connection, but not until JavaScript garbage-collects the object. The
   * connection might stay open longer than you wish if you do not close it explicitly.
   */
  close(): boolean;
   ...
   * 
   * @param port - The TCP/IP port number to listen on. Valid port numbers are 1 to 65535. Typical values are 80 for a
   *               Web server, 23 for a Telnet server and so on.
   ...
   * The call to open() and the call to listen() are mutually exclusive. Call one function or the other, not both.
   * 
   * @param host - The server to connect to. This can be a DNS name, an IPv4 address, or an IPv6 address, followed by a
   ...
   * If a connection request was detected, the method returns a new Socket object that wraps the new connection. Use
   * this connection object to communicate with the remote computer. After use, close the connection and delete the
   * JavaScript object. If no new connection request was detected, the method returns null.
   */
  poll(): Socket;

  /**
   * Reads up to the specified number of characters from the connection. CR characters are ignored unless the encoding
   * is set to "BINARY".
   * Returns a string that contains up to the number of characters that were supposed to be read, or the number of
   * characters read before the connection closed or timed out.
   * 
   * @param count - The number of characters to read. If not supplied, the connection attempts to read as many
   ...
   * Line feeds are recognized as LF or CRLF pairs. CR characters are ignored. Returns a string containing the
   * characters.
   */
  readln(): string;
   ...
   * 
   * @param text - Any number of string values. All arguments are concatenated to form the string to be written. CRLF
   *               sequences are converted to LFs unless the encoding is set to "BINARY".
   */
  write(text: string): boolean;
...
interface QNameConstructor {
   ...
   * 
   * @param uri - The URI, specified as a Namespace object, an existing QName object, or string. If this is a Namespace
   *              object, the URI is set to the namespace URI, and there is no local name. If this is a QName object,
   *              the URI and localName is set to those of that object. If this is a string, the URI is set to that
   *              string.
...
interface NamespaceConstructor {
   ...
   * 
   * @param prefix - The URIprefix, specified as an existing Namespace object, QName object, or string. If this is a
   *                 Namespace or a QName object, the URI and prefix are set to that of the object. If this is a string,
   *                 the prefix is set to that string, and the URI must be specified.
...
interface XML {
   ...
   * If the argument is not XML, creates a new XML element containing the argument as text. The element name of that new
   * XML is the same as the last element in the original XML.
   * 
   ...
   * The existence of attributes, comments, processing instructions and text nodes within an XML object is not
   * significant in determining if it has complex content.
   */
  hasComplexContent(): boolean;

  /**
   * Reports whether this XML object contains simple content.
   * An XML object is considered to contain simple content if it represents a text node, represents an attribute node or
   * if it represents an XML element that has no child elements. XML objects representing comments and processing
   * instructions do not have simple content. The existence of attributes, comments, processing instructions and text
   * nodes within an XML object is not significant in determining if it has simple content.
   */
   ...
   * 
   * @param child1 - The child to insert the other child after. If null, the method inserts child2 before all children
   *                 of this XML object.
   ...
   * This value corresponds to the element name unless the name has a namespace prefix. For example, if the element has
   * the name "ns:tag", the return value is "tag".
   */
  localName(): string;
  ...
  /**
   * Returns the type of this XML object as one of the strings "element", "attribute", "comment",
   * "processing-instruction", or "text".
   */
  nodeKind(): string;

  /**
   * Puts all text nodes in this and all descendant XML objects into a normal form by merging adjacent text nodes and
   ...
   * The root object, as returned by the XML constructor, does not have a parent and returns null. Note that the E4X
   * standard does not define what happens if this XML object is a list containing elements with multiple parents.
   */
  parent(): XML;
   ...
   * Collects processing-instructions with the given name, if supplied. Otherwise, returns an XML list containing all
   * the children of this XML object that are processing-instructions regardless of their name.
   * 
   ...
   * 
   * @param name - The property name. Can be a numeric property name, a name for a set of XML elements, or the
   *               properties wildcard “*”. If this XML object contains no properties that match the name, the method
   *               returns without modifying this XML object.
   * @param value - The XML with which to replace the value of the matching property. Can be an XML object, XML list or
   *                any value that can be converted to a String with toString().
   */
   ...
   * For text and attribute nodes, this is the textual value of the node; for other elements, this is the result of
   * calling the toXMLString() method. If this XML object is a list, concatenates the result of calling toString() on
   * each element.
   */
  toString(): string;
   ...
   * Always includes the start tag, attributes and end tag of the XML object regardless of its content. It is provided
   * for cases when the default XML to string conversion rules are not desired. Interprets the global settings
   * XML.prettyPrint and XML.prettyIndent.
   */
  toXMLString(): string;

  /**
   * Evaluates the given XPath expression in accordance with the W3C XPath recommendation, using this XML object as the
   * context node.
   * 
...
/**
 * An XML list object.
 * In this implementation, an XMLList object is synonymous to the XML object. The constructor accepts an XML list, but
 * everything else works like theXML object.
 */
interface XMLList { }

...
interface Array<T> {
	 ...
	 *
	 * @param start       - The index of the first element to remove. Negative values are relative to the end of the
	 *                      array.
	 ...
	 * @param values      - A list of one or more values to be added to the array starting at index start. Must specify a
...
interface FileConstructor {
	 ...
	 * @param filter      - A filter that limits the types of files displayed in the dialog. In Windows,a filter
	 *                      expression such as "Javascript files:*.jsx;All files:*.*". In Mac OS, a filter function that
	 *                      takes a File instance and returns true if the file should be included in the display, false if
	 *                      it should not.
...
interface File {
	 ...
	 *
	 * @param mode    - The read-write mode, a single-character string. One of these characters: r (read) Opens for
	 *                  reading. If the file does not exist or cannot be found, the call fails. w (write) Opens a file for
	 *                  writing. If the file exists, its contents are destroyed. If the file does not exist, creates a
	 *                  new, empty file. e (edit) Opens an existing file for reading and writing. a (append) Opens an
	 *                  existing file for reading and writing, and moves the current position to the end of the file.
...
interface FolderConstructor {
	 ...
	 *               value stored in the object is the absolute path. The path need not refer to an existing folder. If
	 *               the path refers to an existing file, rather than a folder: The Folder() function returns a File
	 *               object instead of a Folder object. The new operator returns a Folder object for a nonexisting folder
	 *               with the same name.
	 */
...
interface Socket {
	 ...
	 *
	 * @param port     - The TCP/IP port number to listen on. Valid port numbers are 1 to 65535. Typical values are 80 for
	 *                   a Web server, 23 for a Telnet server and so on.
	 ...
	 * The call to open() and the call to listen() are mutually exclusive. Call one function or the other, not both.
	 *
	 * @param host     - The server to connect to. This can be a DNS name, an IPv4 address, or an IPv6 address, followed

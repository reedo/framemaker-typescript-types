...
/**
 * The NaN global property is a predefined variable with the value NaN (Not-a-Number), as specified by the IEEE-754
 * standard.
 */
 ...
 * Create an UTF-8 ASCII encoded version of this string. The string is converted into UTF-8. Every non-alphanumeric
 * character is encoded as a percent escape character of the form %xx, where xx is the hex value of the character. After
 * the conversion to UTF-8 encoding and escaping, it is guaranteed that the string does not contain characters codes
 * greater than 127. The list of characters not to be encoded is -_.!~*'();/?:@&=+$,#. The method returns false on
 * errors.
 *
 ...
 * In the new string, characters of aString that require URL encoding are replaced with the format %xx, where xx is the
 * hexadecimal value of the character code in the Unicode character set.This format is used to transmit information
 * appended to a URL during, for example, execution of the GET method.Use the unescape() global function to translate
 * the string back into its original format. Returns a string which is aString URL-encoded.
 *
 ...
 * Returns true if the expression is a finite number, false otherwise. False if the value is infinity or negative
 * infinity.
 *
 ...
 *
 * @param what      - The string to localize. A ZString-encoded string that can contain placeholder for additional
 *                    arguments in the form %1 to %n.
 ...
 * Parses a string to find the first set of characters that can be converted to a floating point number, and returns
 * that number, or NaN if it does not encounter characters that it can converted to a number.The function supports
 ...
 * Parses a string to find the first set of characters, in a specified base, that can be converted to an integer, and
 * returns that integer, or NaN if it does not encounter characters that it can convert to a number.
 *
 * @param text - The string from which to extract an integer.
 * @param base - The base of the string to parse (from base 2 to base 36). If not supplied, base is determined by the
 *               format of string.
 */
 ...
 *
 * @param namespace - The namespace to use. Omit this parameter to return to the empty namespace. This is either a
 *                    Namespace object or a string.
 */

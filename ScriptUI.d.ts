...
declare class ScriptUI {
  ...
  /**
   * Collects the enumerated values that can be used in the alignment and alignChildren properties of controls and
   * containers.
   ...
   * The presence of ScriptUI.compatability.su1PanelCoordinates means that the application allows backward compatibility
   * with the coordinate system of Panel elements in ScriptUI version 1.
   */
   ...
   * It contains one function, createEvent(), which allows you to create event objects in order to simulate
   ...
   * Creates a new global image object for use in controls that can display images, loading the associated images from
   * the specified resources or image files.
   * 
...
declare class Window extends _Control {
   ...
   * An active control is the one with focus—that is, the one that accepts keystrokes, or in the case of a Button, be
   * selected when the user typesReturn or Enter.
   */
  active: boolean;

  /**
   * Tells the layout manager how unlike-sized children of this container should be aligned within a column or row.
   * Order of creation determines which children are at the top of a column or the left of a row; the earlier a child is
   * created, the closer it is to the top or left of its column or row. If defined, alignment for a child element
   * overrides the alignChildren setting for the parent container. See alignment property for values.
   */
   ...
   * The cancellation key is the Esc key. By default, looks for a button whose name or text is "cancel" (case
   * disregarded).
   */
   ...
   * An array indexed by number or by a string containing an element's name. The length property of this array is the
   * number of child elements for container elements, and is zero for controls.
   */
   ...
   * The frame consists of the title bar and borders that enclose the content region of a window, depending on the
   * windowing system.
   */
   ...
   * The same as [frameBounds.x, frameBounds.y]. Set this value to move the window frame to the specified location on
   * the screen. The frameBounds value changes accordingly.
   */
  frameLocation: Point;
   ...
   * The first time a container object is made visible, ScriptUI invokes this layout manager by calling its layout()
   * function. Default is an instance of the LayoutManager class that is automatically created when the container
   * element is created.
   */
   ...
   * You can specify different margins for each edge of the container. The default value is based on the type of
   * container, and is chosen to match the standard Adobe UI guidelines.
   */
  margins: number;
   ...
   * Interpreted by the layout manager for the container. The default LayoutManager  Object accepts the
   * (case-insensitive) values row, column, or stack.For window and panel, the default is column, and for group the
   * default is row. The allowed values for the container’s alignChildren and its children’s alignment properties depend
   * on the orientation.
   */
  orientation: string;
   ...
   * Because each container holds only a single row or column of children, only a single spacing value is needed for a
   * container. The default value is based on the type of container, and is chosen to match standard Adobe UI
   * guidelines.
   */
  spacing: number;
   ...
   * 
   * @param type - The window type. One of: window: Creates a simple window that can be used as a main window for an
   *               application. (Not supported by Photoshop CS3.) palette: Creates a modeless dialog, also called a
   *               floating palette. (Not supported by Photoshop CS3.) dialog: Creates a modal dialog. This argument can
   *               also be a ScriptUI resource specification; in that case, all other arguments are ignored.
   ...
   * 
   * @param type - The type of the child element, as specified for the type property. Control types are listed in the
   *               JavaScript Tools Guide.
   * @param bounds - A bounds specification that describes the size and position of the new control or container,
   *                 relative to its parent. If supplied, this value creates a new Bounds object which is assigned to
   *                 the new object’s bounds property.
   * @param text - The text or label, a localizable string. Initial text to be displayed in the control as the title,
   *               label, or contents, depending on the control type. If supplied, this value is assigned to the new
   *               object’s text property.
   * @param properties - An object that contains one or more creation properties of the new child (properties used only
   *                     when the element is created). The creation properties depend on the element type. See
   *                     properties property of each control type.
   */
   ...
   * @param title - A string to appear as the title of the dialog, if the platform supports a title. Ignored in Mac OS,
   *                which does not support titles for alert dialogs. The default title string is "Script Alert".
   * @param errorIcon - When true, the platform-standard alert icon is replaced by the platform-standard error icon in
   *                    the dialog. Ignored in Mac OS, which does not support icons for alert dialogs.
   */
   ...
   * 
   * @param message - The string for the displayed message.
   * @param noAsDefault - When true, the No button is the default choice, selected when the user types Enter. Default is
   ...
   * This includes windows defined by ScriptUI resource strings, windows already created by a script, and windows
   * created by the application (if the application supports this case). This function is not supported by all
   * applications. Returns a Window object found or generated from the resource, or null if no such window or resource
   * exists.
   * 
   * @param type - The name of a predefined resource available to JavaScript in the current application; or the window
   *               type. If a title is specified, the type is used if more than one window with that title is found. Can
   ...
   * 
   * @param eventName The event name; if omitted, the default event is sent. One of: onClose, onMove, onMoving,
   ...
   * Called when a request is made to close the window, either by an explicit call to the close() function or by a user
   * action (clicking the OS-specific close icon in the title bar). The function is called before the window actually
   * closes; it can return false to cancel the close operation.
   */
  onClose(): boolean;
   ...
   * Called while a window is being resized, each time the height or width changes. A handler can monitor the resize
   * operation.
   */
  onResizing(): void;

  /**
   * In Windows only, an event-handler callback function, called a shortcut-key sequence is typed that matches the
   ...
   * Called when a request is made to open the window using the show() method, before the window is made visible, but
   * after automatic layout is complete. A handler can modify the results of the automatic layout.
   */
  onShow(): void;
   ...
   * @param title - A string to appear as the title of the dialog. In Windows, this appears in the window’s frame; in
   *                Mac OS it appears above the message. The default title string is "Script Prompt".
   */
...
declare class LayoutManager {
   ...
   * Adjusts sizes and positions of the child elements of this window or container according to the placement and
   * alignment property values in the parent and children.
   * Invoked automatically the first time the window is displayed. Thereafter, the script must invoke it explicitly to
   * change the layout in case of changes in the size or position of the parent or children.
   */
  layout(): void;

  /**
   * Performs a layout after a Window is resized, based on the new size.
   * Resizes the child elements of the managed container with a given alignment type, after the window has been resized
   * by the user.
   */
  resize(): void;
}
 ...
 * Create with ScriptUIGraphics.newPen(). Use as a value of  foregroundColor properties, and pass as an argument to
 * drawString() and strokePath() methods.
 */
declare class ScriptUIPen {

  /**
   * The pen color.
   * The paint color to use when the type is SOLID_COLOR. An array in the form [R, B, G, A] specifying the red, green,
   * blue values of the color and the opacity (alpha channel) value as numbers in the range [0.0..1.0]. An opacity of 0
   * is fully transparent, and an opacity of 1 is fully opaque.
   */
  ...
  /**
   * The theme name.
   * The name of a color theme to use for drawing when the type is THEME_COLOR. Theme colors are defined by the host
   * application.
   */
 ...
 * Create with ScriptUIGraphics.newBrush(). Use as a value of  backgroundColor properties, and pass as an argument to
 * the fillPath() method.
 */
 ...
 * Create with the newPath(), moveto(), lineto(), rectPath(), and ellipsePath() methods.Used as value of currentPath,
 * where it is acted upon by methods such as closePath().Pass as optional argument to fillPath() and strokePath(), which
 ...
 * Allows a script to customize aspects of the element’s appearance, such as the color and font. Use an onDraw callback
 * function to set these properties or call the functions.All measurements are in pixels.
 */
declare class ScriptUIGraphics {
   ...
   * The paint color and style is defined in this brush object.This property is only supported for controls like
   ...
   * Defines a line from the current postion (currentPoint) to the start point of the current path (the value of
   * currentPath).
   */
  closePath(): void;
   ...
   * 
   * @param image - The image to draw. This object contains different versions of an image appropriate to various
   *                element states, such as a dimmed version for the disabled state.
   ...
   * @param width - The width in pixels. If provided, the image is stretched or shrunk to fit. If omitted, uses the
   *                original image width.
   * @param height - The height in pixels. If provided, the image is stretched or shrunk to fit. If omitted, uses the

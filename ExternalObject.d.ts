...
interface ExternalObject {
	...
	/**
	 * A set of alternate paths in which to search for the shared library files, a single string with multiple path
	 * specifications delimited by semicolons (;). Paths can be absolute or relative to the Folder.startup location.
	 */
	...
	/**
	 * Reports whether a compiled C/C++ library can be found, but does not load it. If logging is on, the paths searched
	 * are reported to the JavaScript Console in the ExtendScript Toolkit.
	 ...
	 * It can be helpful to force a shutdown of the external library if termination of external libraries during the
	 * shutdown of the hosting application does not occur in the correct order.
	 */
	terminate(): undefined;
}
